...
def plan_node(state: IState)->State:
    ...
    return {"plan": response.content ,"draft":'',"task":task,"revision_number":0,"max_revisions":2,
...
def generation_node(state: State):
    ...
    for chunk in stream_model.stream(messages):
        ...
        if state.get("revision_number",1)==2:
            response+=chunk.content  # for final report generation
            # writer(chunk.content)
            writer(response) # final streaming 
            #  print(chunk.content)
        response+=chunk.content # for reflect use case
